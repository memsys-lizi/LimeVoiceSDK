...
namespace LimeVoice.Network
{
    public class UdpTransport : IDisposable
    {
        ...
        public void Connect(string host, int port)
        {
            ...
            var addr = System.Array.Find(addresses, a => a.AddressFamily == AddressFamily.InterNetwork)
                       ?? addresses[0];
        ...
        public void Send(byte[] data)
        {
            ...
            catch (Exception e) { Debug.LogWarning($"[LimeVoice] Send error: {e.Message}"); }
        }
        ...
        private void ReceiveLoop()
        {
            while (_running)
            {
                try
                {
                    ...
                    if (PacketParser.TryParse(buf, out var packet))
                        InboundQueue.Enqueue(packet);
                }
                catch (SocketException e)
                {
                    if (_running)
                        Debug.LogWarning($"[LimeVoice] Recv error: {e.Message}");
                }
                catch (Exception e)
                {
                    Debug.LogWarning($"[LimeVoice] Recv unexpected: {e.Message}");
                }
            }
        }
